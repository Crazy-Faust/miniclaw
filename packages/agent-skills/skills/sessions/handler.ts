// In-process handler for the bundled `sessions` skill: sessions_list/history/
// send/spawn, bound to a Gateway. The CLI wires the same Gateway into the
// daemon and into this registry so the agent sees a coherent view of running
// sessions. Needs a runtime Gateway, so the CLI wires it via the
// @miniclaw/agent-skills/runtime subpath rather than through BUILTIN_HANDLERS.
import { z } from "zod";
