// In-process handler for the bundled `cron` skill: reminder_add + cron_add/
// list/remove/pause, bound to a CronStore. The scheduler half (CronScheduler
// from @miniclaw/gateway) reads the same store on a tick to fire jobs. Because
// this needs a runtime CronStore, the CLI wires it via the @miniclaw/agent-skills/runtime
// subpath rather than through BUILTIN_HANDLERS.
import { z } from "zod";
