// In-process handler for the bundled `browser` skill: browser_open / read_page
// / screenshot / click / fill, backed by a lazily-loaded Playwright driver.
// Playwright is an OPTIONAL peer dependency, so the built-in factory only
// registers these tools when `playwright` is actually installed.
...
export type { BrowserDriver, DriverFactory } from "./driver.ts";
export { playwrightDriverFactory } from "./playwright-driver.ts";
...
/**
 * Built-in factory. Browser tools depend on the optional `playwright` peer; we
 * only register them when it is actually installed, so a default install (no
 * playwright) sees no dormant, always-failing browser tools.
 */
export function browserSkills(_env: NodeJS.ProcessEnv): Skill[] {
  try {
    createRequire(import.meta.url).resolve("playwright");
  } catch {
    return [];
  }
  return createBrowserSkills();
}
