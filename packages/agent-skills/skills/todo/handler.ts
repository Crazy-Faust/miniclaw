// In-process handler for the bundled `todo` skill: todo_write maintains a
// multi-step plan that persists across turns of a process. Self-contained
// (in-memory store), so it registers through BUILTIN_HANDLERS.
import { z } from "zod";
import { ok, type Skill } from "@miniclaw/core";
import { InMemoryTodoStore, type TodoStore } from "./store.ts";
...
/**
 * Bind the skill to a specific TodoStore. The built-in factory uses a
 * process-wide store; tests pass their own.
 */
...
// One in-memory plan per process, shared by the registered todo_write tool.
const sharedStore = new InMemoryTodoStore();

/** Built-in factory: the todo_write tool bound to the process-wide store. */
export function todoSkills(): Skill[] {
  return [createTodoWriteSkill(sharedStore)];
}
