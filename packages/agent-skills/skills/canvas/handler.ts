// In-process handler for the bundled `canvas` skill: canvas_create/update/
// list/delete over an in-memory store. Self-contained (no runtime wiring), so
// it registers through BUILTIN_HANDLERS with a module-level store.
import { z } from "zod";
...
export interface CanvasSkillOpts {
   ...
   * compose URLs as `${baseUrl}/canvas/<id>` — pass the externally-reachable
   * origin of the gateway's HTTP server. Defaults to "http://localhost:3000".
   */
  baseUrl?: string;
}
...
// One in-memory store per process, shared by the registered canvas tools.
const sharedStore = new CanvasStore();

/** Built-in factory: canvas tools bound to the process-wide store. */
export function canvasSkills(): Skill[] {
  return createCanvasSkills({ store: sharedStore });
}
