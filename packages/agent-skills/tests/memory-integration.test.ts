...
import { searchMemorySkill, writeMemorySkill } from "../skills/memory/handler.ts";

// Integration: the memory skill paired with the real SQLite store, so FTS5
// retrieval gets exercised end-to-end through the skill surface.

describe("memory skill ↔ memory-sqlite", () => {
  let dir: string;
